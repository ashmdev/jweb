...
export async function POST(request: Request) {
  try {
    ...
    const { partidoId, equipoId, posicion, nombre, email, telefono, costo } = body

    // Validar que se proporcionaron todos los datos necesarios
    if (!partidoId || !equipoId || !posicion || !nombre || !email || !costo) {
    ...
    console.error("Error al invitar nuevo amigo:", error)
